%% RIVF 2026 submission — IEEE conference format, A4, max 6 pages.
%% Deadline: Aug 31, 2026 (EDAS: https://edas.info/N35414). Single-blind.
...
\author{\IEEEauthorblockN{Do Dinh Phuc}
\IEEEauthorblockA{VinUniversity\\
Hanoi, Vietnam\\
24phuc.dd@vinuni.edu.vn}}
...
gains from this recipe~\cite{recode,inferfix,rapgen,ragfix}. This paper
...
repository bugs (BugsInPy~\cite{bugsinpy}, PyBugHive~\cite{pybughive}).
...
projects, complete all three arms cleanly; attrition is project-correlated
(usable/supported: black 26/34, pandas 23/34, salt 3/7, discord.py 1/2,
cookiecutter 1/1, scrapy 1/1, and jax 0/13, freqtrade 0/12, poetry 0/8,
spaCy 0/4 --- lost to dependency-build failures, toolchain incompatibilities,
and test-suite timeouts), so the usable set is dominated by two projects and
conclusions from it inherit that skew (Section~\ref{sec:discussion}).
...
and receives the same exploratory, not-established reading. Controls rule out easy explanations: corpus--benchmark domain match
moves results by small, inconsistent, non-significant amounts; and the
Llama-3-8B prompt-format collapse (123/164 syntax-invalid outputs under the
longer RAG prompt) disappears with robust extraction (0/187), after which
retrieval remains neutral.
...
retrieval-prompt overhead; the cleaner planted-vs-\structured(normal)
contrast (same prompt shape, corpora differing only in the plants) shows
identical pooled totals but different per-project compositions, and
per-case analysis of the 11 discordant baseline-vs-planted cases ---
why exact-fix examples converted 4 yet accompanied 7 regressions --- is
the key open diagnostic our logs do not fully resolve. What the
replication establishes is therefore modest but real: the encouraging
single-project effect was not recovered at scale, and any true pooled
effect of guaranteed coverage on these real bugs is bounded below
$+6.3\pp$ --- far from the $+14$--$22\pp$ the synthetic decomposition
would predict.
...
\textbf{Threats.} \emph{External validity:} the coverage decomposition
...
one model (GPT-4o-mini), one CI run per arm, and cross-environment
variation of the same magnitude as the arm differences (we did not run
repeated CI trials to quantify a flake floor). \emph{Construct validity:}
...
(Section~\ref{sec:results}). Whether the planted pairs were retrieved into
the $k{=}2$ context on the real-bug arms is not instrumented in our CI
logs --- on synthetic bugs the hit rate is 94--100\%, but long repository
files may retrieve differently, and a low hit rate would reframe RQ4's
null as a retrieval failure rather than a conversion failure.
...
attention should go. All code, corpora-construction scripts, per-case
results, and the sharded CI harness for the real-bug replication are
available at
\url{https://github.com/PHUCDODINH/failure-aware-retrieval-repair}.
